    ...
    InternalFilter.prototype._filterLimitCard = function (args) {
        ...
        for (var i = 0; i < kObj.model.columns.length; i++) {
            ...
            if (kObj.model.enableTotalCount) {
                ...
                if (!ej.isNullOrUndefined(args) && (args.requestType == "drop" || (args.requestType == "filtering" ? (args.filterCollection.length > 0 ? true : kObj.model.searchSettings.key == "" ? false : true) : args.requestType == "search" ? (kObj.model.searchSettings.key != "" ? true : (kObj.model.filterSettings.length > 0 ? kObj.element.find(".e-kanbantoolbar .e-tooltxt").hasClass("e-select") : false)) : false))) {
                    var $headerCell = kObj.getHeaderContent().find(".e-columnheader").not(".e-stackedHeaderRow").find(".e-headercell")[i];
...
var InternalDragAndDrop = (function () {
    ...
    InternalDragAndDrop.prototype._dragStop = function (args, clone, pre) {
        ...
        if (kObj._filterCollection.length != 0) {
            ...
            args.filterCollection = kObj._filterCollection;
            kObj.KanbanCommon._processBindings(args);
        }
        else {
            kObj.KanbanCommon._renderLimit();
            ...
            if (kObj.KanbanSwimlane)
                kObj.KanbanSwimlane._swimlaneLimit(args);
        }
        kObj._eventBindings();
        if (kObj.model.isResponsive)
            kObj.kanbanWindowResize();
            ...
            drag: function (args) {
                ...
                if (dropTd && dropTd.hasClass("e-droppable") && (isSwimDragAndDrop ? true : $(dropTd).parent().has(args.element[0]).length > 0) && (target != args.element[0] || selectedCards.length > 1)) {
                    ...
                    if (($(target).hasClass("e-rowcell") || $(target).hasClass("e-kanbancard") || $(target).hasClass("e-targetdragclone") || ($(target).hasClass("e-swimlane-name") || ($(target).hasClass("e-limits"))) || $(target).parents(".e-rowcell").not(".e-drop").length > 0) && $(trgtCloneMulti).is(":visible") && ($(target).index() != trgtCloneMulti.parents(".e-rowcell").index())) {
                        ...
                        if (!$(target).prev().hasClass("e-targetdragclone")) {
    ...
    InternalSwimlane.prototype._swimlaneLimit = function (args) {
        ...
        for (var i = 0; i < kObj.model.columns.length; i++) {
            ...
            if (!ej.isNullOrUndefined(constraints)) {
                ...
                if ((constraints.type == "swimlane" && !ej.isNullOrUndefined(kObj.model.fields.swimlaneKey)) && ((kObj._enableSwimlaneCount && (args.requestType == "filtering" || args.requestType == "refresh" || args.type == "dragStop")) || (kObj._filterCollection.length > 0 && kObj._enableSwimlaneCount))) {
...
var InternalCommon = (function () {
    ...
    InternalCommon.prototype._renderSingleCard = function (primaryKey, card) {
        ...
        if (!kObj._dropped) {
            ...
            if (!ej.isNullOrUndefined(swimlaneKey)) {
                if (ej.isNullOrUndefined(card[swimlaneKey]) || card[swimlaneKey] == "")
                    card[swimlaneKey] = kObj.localizedLabels.Unassigned;
    ...
    InternalCommon.prototype._toggleCardByTarget = function ($target) {
        ...
        if ($target.hasClass("e-cardexpand")) {
            ...
            $cardContent.hide();
            if ($.inArray(primarykey, kObj._collapsedCards) == -1)
                kObj._collapsedCards.push(primarykey);
    ...
    InternalCommon.prototype._kanbanUpdateCard = function (primaryKey, data) {
        ...
        var swimlaneKey = kObj.model.fields.swimlaneKey;
        if (!ej.isNullOrUndefined(swimlaneKey)) {
            if (ej.isNullOrUndefined(data[swimlaneKey]) || data[swimlaneKey] == "")
                data[swimlaneKey] = kObj.localizedLabels.Unassigned;
        }
        ...
        if (new ej.DataManager(kbnCurrentData).executeLocal(new ej.Query().where(kObj.model.keyField, ej.FilterOperators.equal, data[kObj.model.keyField])).length == 0)
            return;
    ...
    InternalCommon.prototype._totalCount = function () {
        ...
        for (var i = 0; i < kObj.model.columns.length; i++) {
            var column = kObj.model.columns[i];
            if (ej.isNullOrUndefined(column.constraints) && kObj.model.enableTotalCount) {
    ...
    InternalCommon.prototype._renderLimit = function () {
        var kObj = this.kanbanObj, total, totalcard, $headerCell = {};
        for (var i = 0; i < kObj.model.columns.length; i++) {
            var column = kObj.model.columns[i];
            if (!ej.isNullOrUndefined(column.constraints)) {
                ...
                total = kObj.getHeaderContent().find("span.e-totalcount");
...
var Kanban = (function (_super) {
    ...
    function Kanban(element, options) {
        ...
        this._keyPressed = function (action, target, e, event) {
            this.KanbanCommon._kanbanKeyPressed(action, target, e, event);
        };
    ...
    Kanban.prototype._showToolTip = function (event) {
        ...
        if (kObj.model.tooltipSettings.enable) {
            ...
            var xPos = !ej.isNullOrUndefined(event.originalEvent) ? event.pageX : event.originalEvent.pageX;
            var yPos = !ej.isNullOrUndefined(event.originalEvent) ? event.pageY : event.originalEvent.pageY;
    ...
    Kanban.prototype._renderHeader = function () {
        ...
        for (var columnCount = 0; columnCount < columns.length; columnCount++) {
            ...
            if ($headerCell.find('.e-clexpand').length > 0) {
    ...
    Kanban.prototype.sendDataRenderingRequest = function (args) {
        ...
        if (this.currentViewData != null && this.currentViewData.length) {
            switch (args.requestType) {
                ...
                case "cancel":
                    ...
                    if (!ej.isNullOrUndefined(this.model.fields.swimlaneKey)) {
                        var proxy = this, swimlaneKey = this.model.fields.swimlaneKey;
                        queryManagar = queryManagar.group(swimlaneKey);
                        if (!this.currentViewData.GROUPGUID) {
                            $.map(proxy.currentViewData, function (obj, index) {
                                var type = obj[proxy.model.fields.swimlaneKey];
                                if (type == "" || type == undefined)
                                    proxy.currentViewData[index][swimlaneKey] = proxy.localizedLabels.Unassigned;
                            });
                            if (!this.currentViewData.GROUPGUID)
        ...
        this._newData = null;
        if (this.model.enableTotalCount && !this._filterCollection.length > 0)
            this.KanbanCommon._totalCount();
    };
    ...
    Kanban.prototype._renderComplete = function (args) {
        ...
        if (args.requestType == "filtering" || args.requestType == "search") {
            this.KanbanFilter._filterLimitCard(args);
            if (this._filterCollection.length == 0) {
                this.KanbanCommon._renderLimit();
                this.KanbanCommon._totalCount();
