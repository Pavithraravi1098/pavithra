...
(function (SaveImageFormat) {
    SaveImageFormat[SaveImageFormat["PNG"] = "png"] = "PNG";
    SaveImageFormat[SaveImageFormat["JPG"] = "jpg"] = "JPG";
    SaveImageFormat[SaveImageFormat["BMP"] = "bmp"] = "BMP";
    SaveImageFormat[SaveImageFormat["TIFF"] = "tiff"] = "TIFF";
...
var ejSignature = (function (_super) {
    ...
    function ejSignature(element, options) {
        ...
        this.defaults = {
            ...
            saveImageFormat: SaveImageFormat.PNG,
            isResponsive: false,
    ...
    ejSignature.prototype.save = function () {
        window.open(this._canvas[0].toDataURL.apply(this._canvas[0]));
    };
    ...
    ejSignature.prototype._render = function () {
        this.element.addClass(" e-signature e-select e-widget").attr("role", "signature");
    };
    ...
    ejSignature.prototype._destroy = function () {
        this.element.removeClass(" e-signature e-select e-widget");
    ...
    ejSignature.prototype._calculateCurveControlPoints = function (p1, p2, p3) {
        var dx1 = p1.x - p2.x, dy1 = p1.y - p2.y, dx2 = p2.x - p3.x, dy2 = p2.y - p3.y, m1 = { x: (p1.x + p2.x) / 2.0, y: (p1.y + p2.y) / 2.0 }, m2 = { x: (p2.x + p3.x) / 2.0, y: (p2.y + p3.y) / 2.0 }, l1 = Math.sqrt(dx1 * dx1 + dy1 * dy1), l2 = Math.sqrt(dx2 * dx2 + dy2 * dy2), dxm = (m1.x - m2.x), dym = (m1.y - m2.y), k = l2 / (l1 + l2), cm = { x: m2.x + dxm * k, y: m2.y + dym * k }, tx = p2.x - cm.x, ty = p2.y - cm.y;
        return {
