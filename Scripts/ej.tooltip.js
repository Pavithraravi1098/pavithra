...
var ejTooltip = (function (_super) {
    ...
    ejTooltip.prototype._wireTriggerEvents = function (val) {
        var wire = (val) ? "_on" : "_off";
        if (ej.isTouchDevice())
            this._wireTouchEvents(val);
        else {
            this._wireMouseEvents(val);
            var triggerEvent = (this.model.trigger == ej.Tooltip.Trigger.Click) ? "click" : (this.model.trigger == ej.Tooltip.Trigger.Focus) ? "focus" : "mouseenter";
    ...
    ejTooltip.prototype._wireTouchEvents = function (val) {
        var wire = (val) ? "_on" : "_off";
        if (ej.isMobile() || ej.isTablet()) {
            (this.model.target != null) ? this[wire](this.element, "tap", this.model.target, this._targetHover) : this[wire](this.element, "tap", this._targetHover);
            if (this.model.associate == ej.Tooltip.Associate.MouseEnter || this.model.associate == ej.Tooltip.Associate.MouseFollow)
                (this.model.target != null) ? this[wire](this.element, "tap", this.model.target, this._tooltipMove) : this[wire](this.element, "tap", this._tooltipMove);
        }
    };
    ejTooltip.prototype._wireMouseEvents = function (val) {
        var wire = (val) ? "_on" : "_off";
        if (this.model.associate == ej.Tooltip.Associate.MouseEnter || this.model.associate == ej.Tooltip.Associate.MouseFollow)
            (this.model.target != null) ? this[wire](this.element, "mousemove", this.model.target, this._tooltipMove) : this[wire](this.element, "mousemove", this._tooltipMove);
    };
    ...
    ejTooltip.prototype._containerCalc = function (target) {
        ...
        this.containerSize = {
            height: (this.model.containment == "body") ? $(window).innerHeight() || document.documentElement.clientHeight || document.body.clientHeight : $(this.model.containment).height(),
            width: (this.model.containment == "body") ? $(window).innerWidth() || document.documentElement.clientWidth || document.body.clientWidth : $(this.model.containment).width()
        };
    ...
    ejTooltip.prototype._mousePosition = function (event) {
        ...
        var tipGapX = 0, tipGapY = 0, tipSize = 0, positionTooltip = $.extend(true, {}, this.model.position.stem), position = $.extend(true, {}, this.model.position), containerElement = { left: 0, top: 0 }, calcPosition = { left: event.pageX, top: event.pageY }, childElement = { left: event.pageX, top: event.pageY }, containerLeft = this.containerSize.left, containerTop = this.containerSize.top;
        var targetLeft = (this.model.containment != "body") ? (event.pageX - $(this.model.containment).offset().left) : event.pagetX, targetTop = (this.model.containment != "body") ? (event.pageY - $(this.model.containment).offset().top) : event.pageY;
    ...
    ejTooltip.prototype._tooltipPosition = function (position) {
        ...
        var tipY = (positionTooltip.vertical === "top" || positionTooltip.vertical === "bottom") ? (5 + (this.tipSize.width / 2)) : 0, tipX = (positionTooltip.horizontal === "right" || positionTooltip.horizontal === "left") ? (10 + (this.tipSize.width / 2)) : 0, tipGap = (positionTooltip.horizontal !== "center") ? this.model.tip.adjust.xValue : (positionTooltip.vertical !== "center") ? this.model.tip.adjust.yValue : 0;
        tipSize = (this.model.isBalloon) ? (positionTooltip.horizontal !== "center") ? this.model.tip.size.height : (positionTooltip.vertical !== "center") ? this.model.tip.size.height : 0 : 2;
