...
(function ($, ej, undefined) {
    ...
    ej.PivotAnalysis = {
                ...
                for (var cnt = 0; cnt < dataSource.data.length; cnt++) {
                    ...
                    for (var nC = 0; nC < calcValues.length; nC++) {
                        ...
                        if (calcValues[nC] != undefined && calcValues[nC] != null) {
                            list[calcValues[nC].fieldName] = this._getNumber($(list).prop(calcValues[nC].fieldName), calcValues[nC].formatString);
        ...
        _getFilteredIndex: function (dataSource, kCnt, keyCol) {
            return $.map(dataSource.values, function (value, index) {
                if (dataSource.rows.length > 0 && keyCol[kCnt].keys.length > 0   &&
                    dataSource.rows[keyCol[kCnt].keys.length - 1].advancedFilter &&
                    dataSource.rows[keyCol[kCnt].keys.length - 1].advancedFilter.length > 0 &&
                    (dataSource.rows[keyCol[kCnt].keys.length - 1].advancedFilter[0].measure == value.fieldName ||
                    dataSource.rows[keyCol[kCnt].keys.length - 1].advancedFilter[0].name == value.fieldName
                    ))
                    return index;
            })[0];
        },
                ...
                for (cCnt = 0; cCnt < this._colKeysCalcValues.length; cCnt++) {
                    ...
                    if (dataSource.enableAdvancedFilter) {
                        ...
                        if (fieldItem != null && fieldItem.axis == "rows")
                            filterIndex = this._getFilteredIndex(dataSource, rCnt, me._rowKeysCalcValues);
                        if (!ej.isNullOrUndefined(filterIndex) && fieldItem != null && fieldItem.axis != "rows") 
                            filterIndex = this._getFilteredIndex(dataSource, cCnt, me._colKeysCalcValues);                                
                        ...
                        if (!ej.isNullOrUndefined(filterIndex)) {
                            ...
                            if (this._colKeysCalcValues[cCnt].uniqueName && this._rowKeysCalcValues[rCnt].keys[0].indexOf("Grand Total") > -1 && isColFiltered)
