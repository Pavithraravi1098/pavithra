...
(function ($, ej, undefined) {
    ...
    ej.widget("ejMaskEdit", "ej.MaskEdit", {
        ...
        _wireEvents: function () {
            ...
            if(ej.isDevice())
            this._on(this.element,'input', this._changeininput);
        },

        _checkflagvalue: function(unicode,e){
        ...
        _validatemaskvalue: function(promptChar,currentPos,actualkey){
        ...
        _changeininput: function (e) {
          
            if (this.model.readOnly) return;
            if(this._masklenth == 0)
                return true
            var promptChar = this.model.showPromptChar ? "_" : " ";
            var currentPos = this._seekNext();
            var unicode = this._textbox.value.charCodeAt(currentPos);
            var actualkey = String.fromCharCode(unicode);
            var initialvalue = this._textbox.value.slice(currentPos, -1);
            var strBeforeCursor = this._textbox.value.substring(0, currentPos);
            var strAfterCursor = this._textbox.value.substring(currentPos);
            this._textbox.value = strBeforeCursor + strAfterCursor.substr(1, strAfterCursor.length);
            if (this._validateChars(unicode, currentPos))
                this._validatemaskvalue(promptChar,currentPos,actualkey); 
            else
                this._ErrorHandler(currentPos);
            if (currentPos >= this.model.maskFormat.length) {
                this._textbox.value = this._textbox.value.substr(0, currentPos);
            }
            this._checkflagvalue(unicode, e);
        },
