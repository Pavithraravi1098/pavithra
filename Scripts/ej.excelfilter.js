...
*/
(function (fn) {
    typeof define === 'function' && define.amd ? define(["./ej.menu"], fn) : fn();
})
(function () {
...
(function ($, ej, undefined) {
    ...
    ej.excelFilter.prototype = {
        ...
        _renderSubCtrls: function (type) {
            ...
            $("#" + this.id + type + "_OkBtn").ejButton({ text: this.localizedLabels.Ok, showRoundedCorner: true, width: 60, click: ej.proxy(this._fltrBtnHandler, this), enabled: true });
        ...
        _setCheckBoxList: function (args) {
            ...
            if (this._currentData.length != 0) {                
                ...
                this._listsWrap.find(".e-selectall").ejCheckBox({ change: ej.proxy(this._selectAllHandler, this), enableTriState: this._interDeterminateState });
        ...
        _checkHandler: function (args) {
            ...
            var $checkedlist = this._listsWrap.find("input.e-ftrchk").filter(":checked:not(.e-selectall)"), clen = $checkedlist.length;
        ...
        _renderCDlgContent: function (content, type) {
            ...
            if (type == "number") {
                ...
                $([$dp1, $dp2]).ejDropDownList({ popupWidth: "170px" });
                $([$in1, $in2]).ejDateTimePicker({ "cssClass": this._ctrlInstance.model.cssClass, height: "27px", width: "177px", "enableRTL": this._ctrlInstance.model.enableRTL, watermarkText: this.localizedLabels.DateTimePickerWaterMark });
            }
            else if (type == "string") {
            ...
            $("#" + $id + "Custom_OkBtn").ejButton({ text: this.localizedLabels.Ok, showRoundedCorner: true, width: "23.6%", click: ej.proxy(this._fltrBtnHandler, this), enabled: true });
        ...
        _getCDlgFields: function () {
            ...
            var $ginputs = this._openedFltr.find(".e-value input"),
                $pred = this._openedFltr.find(".e-predicate input[type='radio']:checked").val();
        ...
        _fltrBtnHandler: function (args) {
            ...
            if (!this._openedFltr.hasClass("e-dlgcustom")) {
                ...
                var checked = this._listsWrap.find("input.e-ftrchk").filter(":checked:not(.e-selectall)");
