...
var RecurrenceEditor = (function (_super) {
    ...
    RecurrenceEditor.prototype._dayOfWeekInMonth = function (strDate, dayName, weekIndex) {
        var dayIndex = this._dayNames.indexOf(dayName);
        ...
        return new Date(weekIndex == 1 || weekIndex == -1 ? curDate.toString() : new Date(curDate.setDate(curDate.getDate() + ((weekIndex - 1) * 7))).toString());
    };
            ...
            switch (this._rRule.freq) {
                ...
                case "MONTHLY":
                    if (!ej.isNullOrUndefined(this._rRule.monthDay)) {
                        ...
                        var _checkMonth = day_start.getMonth();
                        var _lastDate = true;
                        while (new Date(day_start).getDate() != this._rRule.monthDay && _lastDate) {
                            day_start = new Date(day_start.setDate(day_start.getDate() + 1));
                            _lastDate = _checkMonth == day_start.getMonth() ? true : false;
                        }
                        if (!_lastDate)
                            day_start = new Date(day_start.setDate(day_start.getDate() - 1));
                    }
        ...
        for (var r = 0; r < _rRuleSplit.length; r++) {
            ...
            switch ($.trim(_rule[0]).toUpperCase()) {
                ...
                case "COUNT":
                    ...
                    break;
                case "UNTIL":
                    this._rRule.until = new Date(_rule[1]);
                    break;
                case "BYDAY":
    ...
    RecurrenceEditor.prototype.clearRecurrenceFields = function () {
        ...
        for (var i = 0; i < list.length; i++) {
            $(list[i]).ejCheckBox({ checked: false });
        }
    ...
    RecurrenceEditor.prototype.showRecurrenceSummary = function (appId) {
        ...
        switch (this._rRule.freq) {
            case "DAILY":
                _recurType.ejDropDownList({ selectedItemIndex: 0 });
                _recurRule += this._getLocalizedLabels("RecurrenceDay").toLowerCase();
                this._rRuleFreq = "DAILY";
                break;
            case "WEEKLY":
                _recurType.ejDropDownList({ selectedItemIndex: 1 });
                this._rRuleFreq = "WEEKLY";
                ...
                break;
            case "MONTHLY":
                _recurType.ejDropDownList({ selectedItemIndex: 2 });
                this._rRuleFreq = "MONTHLY";
                if (!ej.isNullOrUndefined(this._rRule.monthDay)) {
                ...
                break;
            case "YEARLY":
                _recurType.ejDropDownList({ selectedItemIndex: 3 });
                this._rRuleFreq = "YEARLY";
                if (!ej.isNullOrUndefined(this._rRule.monthDay)) {
