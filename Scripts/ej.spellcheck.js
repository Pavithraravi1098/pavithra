...
*/
(function (fn) {
    typeof define === 'function' && define.amd ? define(["./../common/ej.core","./../common/ej.data","./../common/ej.scroller","./ej.dialog","./ej.button","./ej.listbox","./ej.menu"], fn) : fn();
})
(function () {
...
var SpellCheck = (function (_super) {
    ...
    SpellCheck.prototype.validate = function () {
        if (this.model.contextMenuSettings.enable && !ej.isNullOrUndefined(this.model.dictionarySettings.dictionaryUrl) && this._statusFlag) {
            var targetText = ej.isNullOrUndefined($(this.element)[0].value) ? ($(this.element)[0].innerText || $(this.element)[0].textContent).trim() : $(this.element)[0].value.trim();
            this._ajaxRequest(this, targetText, "validateOnType");
        }
    };
    ...
    SpellCheck.prototype._dialogWindowRendering = function () {
        ...
        this._spellCheckWindow.find(".e-suggesteditems").ejListBox({ width: "295px", height: "90px", dataSource: null, selectedIndex: 0 });
    ...
    SpellCheck.prototype._showDialog = function () {
        ...
        if (ej.browserInfo().name === "mozilla") {
            this._spellCheckWindow.find(".e-sentence .e-sentencecontent")[0].textContent = targetText;
        }
        else {
            this._spellCheckWindow.find(".e-sentence .e-sentencecontent")[0].innerText = targetText;
        }
        var dialogEvent = { targetText: targetText, requestType: "dialogOpen" };
        if (this._trigger("dialogOpen", dialogEvent)) {
            return false;
        }
        this._ajaxRequest(this, targetText, "spellCheckDialog");
    };
    SpellCheck.prototype._ajaxRequest = function (proxy, text, requestType) {
        var value = requestType === "addToDictionary" ? proxy._customWord : this._getModelValues(this, text, requestType);
        $.ajax({
                        ...
                        if (!ej.isNullOrUndefined(proxy._errorHTML)) {
                            if (!ej.isNullOrUndefined(proxy._spellCheckWindow) &&
                                ...
                                if (proxy._errorWordsData.length > 0) {
                                    ...
                                    var spanElement = contentElement.children("span");
    ...
    SpellCheck.prototype._dialogModeOperations = function (proxy, errorWordData, elementText, requestType) {
        ...
        if (errorWordData.length > 0) {
            proxy._removeSpan(proxy);
            var targetText = proxy.element[0].innerHTML;
            elementText = proxy._formHtml(errorWordData, targetText);
            proxy._errorHTML = contentElement[0].innerHTML = elementText;
    ...
    SpellCheck.prototype._validateOnTypeOperations = function (proxy, errorWordData, elementText, requestType, element) {
        if (errorWordData.length > 0) {
            ...
            var targetText = proxy.element[0].innerHTML;
            elementText = proxy._formHtml(errorWordData, targetText);
            proxy.element[0].innerHTML = elementText;
            $(proxy.element[0]).find("span").addClass(this.model.misspellWordCss);
    ...
    SpellCheck.prototype._removeSpan = function (proxy) {
        var spanElement = $(proxy.element[0]).find("span");
        for (var i = 0; i < spanElement.length; i++) {
            var spanText = spanElement[i].innerText || spanElement[i].textContent;
            $(spanElement[i]).replaceWith(spanText);
        }
    ...
    SpellCheck.prototype._listBoxDataUpdate = function (proxy) {
        ...
        if (filterData[0]["SuggestedWords"].length > 0) {
            if (proxy._spellCheckWindow.find(".e-btnchange").hasClass("e-disable") && proxy._spellCheckWindow.find(".e-btnchangeall").hasClass("e-disable")) {
            ...
            var count = proxy.model.maxSuggestionCount < filterData[0]["SuggestedWords"].length ? proxy.model.maxSuggestionCount : filterData[0]["SuggestedWords"].length;
    ...
    SpellCheck.prototype._renderMenu = function (proxy) {
        var length;
        length = $($(proxy.element)[0]).children("span").length;
    ...
    SpellCheck.prototype._close = function () {
        if (!ej.isNullOrUndefined(this._spellCheckWindow) && this._spellCheckWindow.parents().find(".e-spellcheckparent").length > 0) {
            ...
            var event = { updatedText: contentAreaElement.text(), requestType: "dialogClose" };
                ...
                switch (prop) {
                    ...
                    case "misspellWordCss":
                        ...
                        if (this.model.contextMenuSettings.enable) {
                            var oldMisspellWordCssClass = $(this.element[0]).find("span").attr("class").toString().split(" ")[1];
                            $(this.element[0]).find("span").removeClass(oldMisspellWordCssClass).addClass(this.model.misspellWordCss);
                        }
                        break;
    ...
    SpellCheck.prototype._getModelValues = function (proxy, targetText, requestType) {
        var spellModel = {
