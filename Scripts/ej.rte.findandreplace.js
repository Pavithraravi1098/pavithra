	...
	_findAndReplaceClick:function (args){
		...
		switch(targetElement.getAttribute('ej-function'))
		{
			...
			case "replace":
					...
					if(!this._findObject.nodeCollection.length)	this._selectedIndex=-1;
			...
			break;
			case "replaceAll":
					this._findObject.replaceAll(replacetxt);this._selectedIndex=-1;
					$.merge(navElement,findElement).ejButton("disable");					
					this._setBackupData(); 
			break;		
		}
...
(function() {
  ...
  ej.FindAndReplace.prototype.find=function(text,node,igncase,wholeword){
	if(this.nodeCollection.length)
		this.revertAll();
	this._validateTextContent(text,this._getFormatedText(node),igncase,wholeword);
...
else 
{
 ...
 ej.FindAndReplace.prototype._getFormatedText=function (element){
	var text='';
