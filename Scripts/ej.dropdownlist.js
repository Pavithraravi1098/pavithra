    ...
    ej.widget("ejDropDownList", "ej.DropDownList", {
        ...
        _destroy: function () {
            ...
            if (!this.model.dataSource) {
                this.selectOptions ? this.docbdy && this.docbdy.append(this.selectOptions.show()).show() && this.element.remove() && this.selectOptions && this.selectOptions.removeClass("e-dropdownlist e-js") : this.docbdy && this.ultag.find("li").removeClass("e-active") && this.docbdy.append(this.ultag.html()).show();
            }
            this._hideResult();
        ...
        _OnSearchEnter: function (e) {
            ...
            if ($.inArray(keycode, [38, 40, 13]) != -1 && this.ultag.find("li.e-nosuggestion").length <= 0) {
                if (keycode == 13) this._OnKeyUp(e);
        ...
        _onScroll: function (e) {
            ...
            if (proxy.model.allowVirtualScrolling && proxy.model.virtualScrollMode == "continuous") {
                ...
                if (scrollerPositon >= $(proxy.popupList).find("ul,ol").height() - $(proxy.popupList).height() && proxy._rawList.length < proxy._totalCount) {
                    proxy._addLoadingClass();
        ...
        _onValueChange: function () {
            ...
            if (this.value() != this.element.val() && !(this.value() == null && this.element.val() =="" )) {
                this._updateSelectedIndexByValue(this._visibleInput.val());
                this._updateValue(this.element.val());
        ...
        _removeTextBoxValue: function () {
			...
			this.checkedStatus = false;            
            this._onValueChange();
            if ((this.model.cascadeTo != null) && !this._isSingleSelect() ) this._cascadeAction();
            this.model.checkAll = false;
            this._setWatermark();
        },
        ...
        _getFilteredList: function (list) {
            if (!list || !list.length || list.length < 1) {
                ...
                this._rawList = this.popupListItems;
            }
        },
        ...
        _appendCheckbox: function (listItems, isAddItem) {
            ...
            for (var i = 0; i < listItems.length; i++) {
                ...
                this._setAttr($checkbox, { type: "checkbox", name: "list" + index, "data-role": "none", id: this._id + "_" + "check" + index })
                    ...
                    ._setAttr($spanCheck, { name: "list" + index + "_wrap", "data-role": "none", id: this._id + "_" + "check" + index + "wrap", unselectable: "on" })
