    ...
    ej.widget("ejPivotChart", "ej.PivotChart", {
                        ...
                        for (var mCnt = 0; mCnt < measureRCnt; mCnt++) {
                            if (jsonData.points_Y[(lblVal + mCnt)] != undefined && jsonData.chartLables[index] == jsonData.points_Y[(lblVal + mCnt)][0].Item1)
                                for (var i = 0; i < jsonData.points_Y[index].length; i++) {
                                    points = { "xValues": jsonData.chartLables[index], "yValues": (jsonData.points_Y[lblVal + mCnt][i].Item2).indexOf(",") > -1 ? ej.globalize.parseFloat(ej.globalize.format((jsonData.points_Y[lblVal + mCnt][i].Item2), "c")) : (jsonData.points_Y[lblVal + mCnt][i].Item2) == "" ? 0 : ej.globalize.parseFloat((jsonData.points_Y[lblVal + mCnt][i].Item2)), "text": (jsonData.points_Y[lblVal + mCnt][i].Item2) == "" ? 0 : (jsonData.points_Y[lblVal + mCnt][i].Item2) };
                                    YPoints.push(points);
                                }
                        }
                    }
        ...
        exportPivotChart: function (exportOption, fileName, exportFormat) {
            ...
            if (this.model.operationalMode == ej.PivotChart.OperationalMode.ClientMode) {
                ...
                this.doPostBack(this.model.url + "/" + this.model.serviceMethodSettings.exportPivotChart, params);
            }            
        },
