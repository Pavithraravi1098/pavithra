...
(function ($, ej, undefined) {
    ...
    ej.widget("ejPivotGrid", "ej.PivotGrid", {
        ...
        _endColResizing: function (e) {
           ...
           if (parseInt(_extra) + parseInt(_oldWidth) > _colMinWid) {
                  if (_extra != 0){
                    ...
                    var pos = ((this.model.frozenHeaderSettings.enableFrozenHeaders || (!this.model.frozenHeaderSettings.enableFrozenColumnHeaders && this.model.frozenHeaderSettings.enableFrozenRowHeaders)) && this._JSONRecords[0].CSS == "none") ? parseInt($(_outerCell).attr("p").split(",")[0]) - this._JSONRecords[0].ColSpan : parseInt($(_outerCell).attr("p").split(",")[0]);
           ...
           if ($(_outerCell).next().length) {
                     ...
                     var pos1 = (this.model.frozenHeaderSettings.enableFrozenHeaders || (!this.model.frozenHeaderSettings.enableFrozenColumnHeaders && this.model.frozenHeaderSettings.enableFrozenRowHeaders)) ? parseInt($(_outerCell).next().attr("p").split(",")[0]) - this._JSONRecords[0].ColSpan : parseInt($(_outerCell).next().attr("p").split(",")[0]);
        ...
        _createDialog: function (title, treeViewData) {
            ...
            this.element.find(".e-dialog, .clientDialog").remove();
            var currentHierarchy, levelInfo,  groupDropDown = "", filterElementTag = "";
            ...
            if (isAdvancedFilter) {
                ...
                filterElementTag = ej.buildTag("div.filterElementTag")[0].outerHTML;
                var filterTag = ej.Pivot.createAdvanceFilterTag({ action: "filterTag" }, this)
            }
            var dialogContent = ej.buildTag("div#EditorDiv.editorDiv", groupDropDown + ej.buildTag("div", filterElementTag + ej.buildTag("div.memberEditorDiv", ej.buildTag("div#editorTreeView.editorTreeView")[0].outerHTML)[0].outerHTML)[0].outerHTML)[0].outerHTML + "</br>",
        ...
        _groupLabelChange: function (args) {
            ...
            if (this.model.operationalMode == ej.Pivot.OperationalMode.ClientMode) {
                ...
                var filterMenuObj = this.element.find(".filterElementTag").data("ejMenu");                
               ...
               this.element.find("#clearAllFilters a").text(this._getLocalizedLabels("ClearFilter") + " from \"" + args.text + "\"").append(filterElem);
                ...
                if (filterData.length > 0) {
                    if (filterData[0]["advancedFilterType"] == ej.olap.AdvancedFilterType.LabelFilter) {
                        ...
                        this.element.find("#labelFilterBtn li#" + filterData[0]["labelFilterOperator"] + " a").append(activeFilter);
                        ...
                        this.element.find("#valueFilterBtn li#" + filterData[0]["valueFilterOperator"] + " a").append(activeFilter);
                ...
                if (filterInfo.length > 0 && !ej.isNullOrUndefined(filterInfo[0]["operator"])) {
                    ...
                    if (filterInfo[0]["operator"].replace(/ /g, '') == "BottomCount")
                        this.element.find("#" + filterTag + " li#" + filterInfo[0]["operator"].replace(/ /g, '').replace("Bottom", "top") + " a").append($(ej.buildTag("span.activeFilter e-icon")[0].outerHTML));
                    else
                        this.element.find("#" + filterTag + " li#" + filterInfo[0]["operator"].replace(/ /g, '') + " a").append($(ej.buildTag("span.activeFilter e-icon")[0].outerHTML));
                }
                else {
        ...
        _nodeCheckChanges: function (args) {
            ...
            if ((args.id.toLowerCase() == "all") && args.type == "nodeUncheck") {
            ...
            else if ((args.id.toLowerCase() == "all") && args.type == "nodeCheck") {
                this._memberTreeObj.model.nodeCheck = "";
                this._memberTreeObj.model.nodeUncheck = "";
            ...
            else if (args.id.toLowerCase() != "all") {
        ...
        _filterBtnClick: function (args) {
            ...
            if (this.model.operationalMode == ej.Pivot.OperationalMode.ClientMode) {
                if (this.model.analysisMode == ej.Pivot.AnalysisMode.Olap) {
                    ...
                    this._dialogHead = $(args.target).siblings(".pvtBtn").text();
                ...
                this._dialogHead = this.model.analysisMode == ej.Pivot.AnalysisMode.Olap ? args.target.parentElement.title : $($(args.target).siblings()[1]).text();
                var report;
                try {
                    report = JSON.parse(this.getOlapReport()).Report;
                }
		...
		_drillThroughCellClick: function (args) {
            ...
            if (this.model.operationalMode == ej.PivotGrid.OperationalMode.ServerMode) {
                this._waitingPopup.show()
                this.doAjaxPost("POST", this.model.url + "/" + this.model.serviceMethodSettings.drillThroughHierarchies, JSON.stringify({ "currentReport": JSON.parse(this.getOlapReport()).Report, "layout": this.model.layout, "cellPos": cellPos, "customObject": JSON.stringify(this.model.customObject) }), this._createDrillThroughDialog)
            }
            else {
                var rowSpan = $("#" + this._id).find("tbody").find('tr:first').find('th').length;
                for (i = 0; i < rowSpan; i++) {
                    var rowInfo = this.getJSONRecords()[parseInt((i * this._rowCount) + parseInt(cellPos.split(",")[1]))].Info.split("::")[0];
                ...
                for (i = 0; i < columnHeaderCount; i++) {
                    var colInfo = this.getJSONRecords()[parseInt((parseInt(cellPos.split(",")[0]) * this._rowCount) + i)].Info.split("::")[0];
        ...
        _createDrillThroughDialog: function (args, dataSourceInfo) {
            ...
            this.element.find(".clientDialog").ejDialog({ width: 600, target: "#" + this._id, enableResize: false, enableRTL: this.model.enableRTL, close: ej.proxy(ej.Pivot.closePreventPanel, this) });
            this.element.find(".cubeTreeViewHierarchy").ejTreeView({
                showCheckbox: true,
                fields: { id: "id", parentId: "pid", text: "name", isChecked: "isSelected", spriteCssClass: "spriteCssClass", dataSource: this.model.operationalMode == ej.PivotGrid.OperationalMode.ServerMode? $.parseJSON(args.d) : dataSourceInfo },
            ...
            for (var i = 0; i < treeViewElements.length; i++) {
                var tagValue = this.model.operationalMode == ej.PivotGrid.OperationalMode.ServerMode ? $.parseJSON(args.d)[i].tag : dataSourceInfo[i].tag
        ...
        exportPivotGrid: function (exportOption, fileName) {
            var exportSetting = { url: "", fileName: "PivotGrid", exportMode: ej.PivotGrid.ExportMode.JSON };
            ...
            if (!ej.isNullOrUndefined(axisItems)) {
                ...
                if (this.model.operationalMode == ej.PivotGrid.OperationalMode.ClientMode) {
                    ...
                    for (var i = 0; i < axisItems.length; i++) {
                        ...
                        rowBtns += ej.buildTag("span.pivotButton", ej.buildTag("button.pvtBtn#pivotButton_" + axisItems[i].fieldName || axisItems[i], (1 == 1 ? (axisItems[i].fieldCaption == undefined ? axisItems[i].fieldName : axisItems[i].fieldCaption || axisItems[i]) : $.grep(this._pivotTableFields, function (item) { return item.name == axisItems[i].fieldName; })[0].caption || $.grep(this._pivotTableFields, function (item) { return item.name == axisItems[i]; })[0].caption), {}, { "fieldName": axisItems[i].fieldName, "axis": tagAxis })[0].outerHTML +
                    ...
                    for (var i = 0; i < axisItems.length; i++) {
                        ...
                        if (this.model.operationalMode == ej.PivotGrid.OperationalMode.ServerMode) {
                            ...
                            rowBtns += ej.buildTag("span.pivotButton", ej.buildTag("span.dropIndicator")[0].outerHTML + ej.buildTag("button.pvtBtn#pivotButton" + axisItems[i].FieldHeader || axisItems[i].FieldName || axisItems[i].Name || axisItems[i].DimensionHeader || axisItems[i].DimensionName, (axisItems[i].FieldHeader || axisItems[i].FieldName || axisItems[i].Name || axisItems[i].DimensionHeader || axisItems[i].DimensionName), {}, this.model.analysisMode == ej.Pivot.AnalysisMode.Pivot ? { "fieldname": axisItems[i].FieldName || axisItems[i].DimensionName || axisItems[i].FieldHeader || axisItems[i].DimensionHeader } : {})[0].outerHTML +
        ...
        _createFields: function (tableOlapGrid, tableWidth, cellWidth) {
            ...
            if (this.element.find(".pivotGridTable thead").length > 0 || this.model.enableDeferUpdate && this.element.find(".pivotGridTable thead").length == 0 && this.model.operationalMode == ej.PivotGrid.OperationalMode.ServerMode || this.model.frozenHeaderSettings.enableFrozenHeaders || this.model.frozenHeaderSettings.enableFrozenColumnHeaders || this.model.frozenHeaderSettings.enableFrozenRowHeaders) {
                ...
                if (this.element.find(".pivotGridTable").find("thead th:eq(1)").length > 0 || this.model.enableDeferUpdate && this.element.find(".columns .pivotButton").length == 0 || this.model.frozenHeaderSettings.enableFrozenHeaders || this.model.frozenHeaderSettings.enableFrozenColumnHeaders || this.model.frozenHeaderSettings.enableFrozenRowHeaders) {
        ...
        renderControlFromJSON: function () {
            ...
            if (((jsonObj == null || jsonObj == "") && !this.model.enableGroupingBar)) {
                ...
                if (this.model.frozenHeaderSettings.enableFrozenHeaders || (this.model.frozenHeaderSettings.enableFrozenRowHeaders && !this.model.frozenHeaderSettings.enableFrozenColumnHeaders) || (this.model.frozenHeaderSettings.enableFrozenRowHeaders && this.model.frozenHeaderSettings.enableFrozenColumnHeaders))
                    ...
                    if (this.model.enableColumnResizing && (!this.model.frozenHeaderSettings.enableFrozenColumnHeaders || !this.model.frozenHeaderSettings.enableFrozenRowHeaders || !this.model.frozenHeaderSettings.enableFrozenHeaders)) {
                        ...
                        for (var column = index; column < jsonObj.length; column = column + this._rowCount) {
                            if (jsonObj[column].Span != "Block") {
                                if (jsonObj[column].CSS == "rowheader" || jsonObj[column].CSS == "summary row" || jsonObj[column].CSS == "summary rstot" || jsonObj[column].CSS == "summary rgtot" || jsonObj[column].CSS == "summary stot" || jsonObj[column].CSS == "summary gtot" || jsonObj[column].CSS == "summary") {
									...
									if(jsonObj.length > column + this._rowCount) {
										if ((jsonObj[column + this._rowCount].CSS != "rowheader" && jsonObj[column + this._rowCount].CSS != "none") || (jsonObj[column + this._rowCount].RowSpan <= 1 && jsonObj[column].Value != jsonObj[column + 1].Value && (jsonObj[column].Expander != 1 || jsonObj[column + 1].CSS != "summary rstot"))) {
											tRow1 += "</tr>";
											$(tbodyFrozenRow).append(tRow1);
											tRow1 = "<tr role='row'>";
										}
        ...
        _applyFrozenHeaderWidth: function (jsonObj, drillAction) {
            if(jsonObj.length != 0){
                ...
                if (!this.model.frozenHeaderSettings.enableFrozenRowHeaders && this.model.frozenHeaderSettings.enableFrozenColumnHeaders) {
                    ...
                    this.element.find(".pivotGridValueTable, .pivotGridFrozenTable").append(colgroup).css({ "table-layout": "fixed" });
                    //if(_isNoRow) 
