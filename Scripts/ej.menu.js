    ...
    ej.widget("ejMenu", "ej.Menu", {
        ...
        _addArrow: function (topArrows, bottomArrows) {
            if (topArrows) {
				...
				this.element.find('>li.e-list:has("> ul")').children('a,span').append($('<span>').addClass("e-icon "+arrowIcon)).addClass("e-arrow-space");
			}
            else {
            ...
            if (bottomArrows)
                this.element.find('>li.e-list > ul li.e-list:has(>ul)').children('a,span').append($('<span>').addClass("e-icon e-arrowhead-right")).addClass("e-arrow-space");
            else {
        ...
        _subMenuPos: function (element, direction) {
            ...
            if (this.model.menuType == "normalmenu") {
                if ($(element.parentNode).is(this.element)) {
                    ...
                    else if ((winHeight - pos.top) < submenuHeight) {
                        var menuPos = (winHeight - pos.top) - submenuHeight;
        ...
        _onKeyDownHandler: function (e) {
            ...
            var element, focusEle, itemId = "", hoverElement = this.element.find(".e-mhover"), focusedElement = this.element.find(".e-mfocused"), currentElement;
            ...
            if (e.keyCode == 37) {
                ...
                else if (hoverElement.parent("ul.e-menu").length == 0 || (this.model.menuType == "contextmenu" && hoverElement.parent("ul.e-context").length == 0)) {
                        this._hideAnimation(hoverElement.parent(), this._hideAnim);
                        hoverElement.removeClass("e-mfocused e-mhover");
                        $(focusedElement[focusedElement.length - 1]).removeClass("e-mfocused");
                        liVisible = hoverElement.parent().closest('.e-list').parent().children('li.e-list:visible:not(.e-hidden-item, .e-disable-item)');
