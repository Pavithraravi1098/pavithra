...
(function ($, ej, undefined) {
    ...
    ej.widget("ejAccordion", "ej.Accordion", {
        ...
        _renderControl: function () {
			...
			// Add header icons
            ej.buildTag("span.e-icon " + this.model.customIcon.header).prependTo(this._headers);
        ...
        _setHeaderSize: function () {
            ...
            this._headers.each(function () {
                ...
                if ($(this).find(".e-icon").length > 0) {
                    $(this).find(".e-icon").css("margin-top", ($(this).height() / 2 - 5));
                }
            });
        ...
        _addDeleteIcon: function(){
            if(this.model.showCloseButton){
                ...
                this._on(this._headers.children("div.e-close"), "click", this._panelDeleteClick);
            }else{
                this._headers.children("div.e-close").remove();
                this._off(this._headers.children("div.e-close"), "click", this._panelDeleteClick);
            }
        },
        _createDeleteIcon: function(){
            var deleteIcon = ej.buildTag('div.e-icon e-close');
            return deleteIcon;
        },
        _getHeight: function () {
        ...
        _activeTab: function (index, raiseEvent, isCode) {
            ...
            if ($.inArray(index, this.model.disabledItems) < 0) {
                ...
                if(!ej.isNullOrUndefined(this.model.selectedItemIndex)){
                    ...
                    this._activeHeader.find("span.e-icon")
            ...
            if (this._headers.find(".e-icon").length <= 0) {
                ...
                if (icons) {
                    if (!(this.collapseALL && this.model.collapsible)) {
                        this._headers.eq(this.model.selectedItemIndex).find("span.e-icon")
                        .removeClass(icons.header)
                        .addClass(icons.selectedHeader);
                    }
                }
        ...
        addItem: function (header_name, content, index, isAjaxReq) {
            ...
            this._headers = this.element.find("> :even");
            this._contentPanels = this._headers.next();
            ej.buildTag("span.e-icon " + this.model.customIcon.header).prependTo(headerTag);
            if(this.model.showCloseButton) {
