    ...
    ej.util = {
        ...
        setCulture: function (culture) {
            if (!ej.isNullOrUndefined(ej.globalize))
                ej.preferredCulture(culture)
            else {
                if (ej.isNullOrUndefined(ej.cultures)) ej.cultures = { "en-US": { name: "en-US" } };
                this.globalCultureObject = culture && !ej.isNullOrUndefined(ej.cultures[culture]) ? ej.cultures[culture] : ej.cultures["en-US"];
            ...
            for (var i = 0; i < this.length; i++) {
                ...
                if (!isAlreadyExists) {
                    ...
                    if (!options || typeof options === "object") {
                        if (proto.prototype.defaults && "locale" in proto.prototype.defaults && pluginName != "ejChart") {
                            if (ej.isNullOrUndefined(options)) {
                                options = {};
                            }
                            if (options && !("locale" in options)) {
                                ej.setCulture();
                                options.locale = ej.cultures.current.name;
                            }
                        }
