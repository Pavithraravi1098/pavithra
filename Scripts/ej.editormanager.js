...
(function (documentManager) {
    var editorManager = (function () {
        ...
        editorManager.prototype.justifyContent = function (Value) {
        ...
        editorManager.prototype.applyFormateBlock = function (Value) {
        ...
        editorManager.prototype.applyList = function (value, listType) {
            this._backUpRange(this.currentDocument, this.currentWindow);
            this._getModifedRange(this._getRangy(this.currentWindow, this.currentDocument));
            this._applyOrderedList(value.toLowerCase() == "orderlist" ? "ol" : "ul", this.currentDocument, this.currentWindow, listType);
            this._resetRange(this.currentDocument, this.currentWindow);
        };
        editorManager.prototype.applyIndent = function (value) {
            this._backUpRange(this.currentDocument, this.currentWindow);
            this._getModifedRange(this._getRangy(this.currentWindow, this.currentDocument));
            if (value)
            ...
            for (var i = 0; i < selNode.length; i++) {
                if (this._isTextNode(selNode[i])) {
                    ...
                    if (selNode[i].parentNode.nodeName.toLowerCase() == "body")
                        _parentNode.push(this.updateParent(selNode[i]));
                }
        ...
        editorManager.prototype.updateParent = function (node) {
        ...
        editorManager.prototype._validateFontSize = function (element) {
            var size = parseInt($(((element.nodeType == 3) ? element.parentElement : element)).css("font-size"));
        ...
        editorManager.prototype.tagFormatProcess = function (format, remove) {
        ...
        editorManager.prototype.cssFormatProcess = function (format, styleData, remove) {
        ...
        editorManager.prototype.execCommand = function (command, value) {
            try {
                switch (command.toLowerCase()) {
                    case "bold":
                        this.tagFormatProcess(command.toLowerCase(), false);
                        break;
                    case "italic":
                        this.tagFormatProcess(command.toLowerCase(), false);
                        break;
                    ...
                    case "subscript":
                        ...
                        break;
                    case "fontsize":
                        this.cssFormatProcess(command.toLowerCase(), value, false);
                        break;
                    case "fontname":
                        this.cssFormatProcess(command.toLowerCase(), value, false);
                        break;
                    ...
                    case "backcolor":
                        ...
                        break;
                    case "uppercase":
                        this._caseConvert(command.toLowerCase());
                        break;
                    case "lowercase":
                        this._caseConvert(command.toLowerCase());
                        break;
                    ...
                    case "justifycenter":
                        ...
                        break;
                    case "justifyright":
                        this.justifyContent("right");
                        break;
                    case "justifyfull":
                        this.justifyContent("justify");
                        break;
                    case "formatblock":
                        this.applyFormateBlock(value);
                        break;
                }
    ...
    var RangeExtractor = (function () {
        ...
        RangeExtractor.prototype._insertCursor = function (startContainer, startOffset) {
            ...
            firstNodeData && $(document.createTextNode(firstNodeData)).insertBefore(startContainer);
            lastNodeData && $(document.createTextNode(lastNodeData)).insertAfter(startContainer);
        ...
        RangeExtractor.prototype._collectCursorBasedTextNode = function (startContainer, startOffset) {
            ...
            if (tempLeftIndex && tempRightIndex) {
                if ((startOffset - (tempLeftIndex.index + 1) == 0 || startOffset - (tempLeftIndex.index + 1) == startContainer.data.substring(tempLeftIndex.index + 1, tempRightIndex.index).length)) {
                ...
                this._nodeCollection = [];
                if ((rightSideNode.length == 0) && (((startOffset - (tempLeftIndex.index + 1)) == 0) || ((startOffset - (tempLeftIndex.index + 1)) == startContainer.data.substring(tempLeftIndex.index + 1).length))) {
                    this._insertCursor(startContainer, startOffset);
                }
                else {
                ...
                this._nodeCollection = [];
                if (leftSideNode.length == 0 && ((startOffset == startContainer.data.substring(0, tempRightIndex.index).length) || startOffset == 0)) {
                    this._insertCursor(startContainer, startOffset);
                }
                else {
                ...
                if (startContainer.nodeType != 3) {
                    ...
                    leftSideNode = this._nodeCollection;
                    this._nodeCollection = [];
                    if ((leftSideNode.length == 0 && startOffset == 0) || (rightSideNode.length == 0 && tempNode.data.replace(String.fromCharCode(65279), " ").length == startOffset)) {
                        this._insertCursor(startContainer, startOffset);
                    }
                    else {
                        this._nodeCollection = leftSideNode.reverse();
